function corrected_stat = correct_ftd(settingsfile)         
...
[network,stations,first_day,last_day,channels,location,num_stat_cc,Fq,...
    fileName,fileformat,pz_file,dateformat,deci,missingfiles,bpf,norm,...
    wl,swl,perco,filenameO,fileformatO,dateformatO,datesm,fit...
...
for jj = 1:nost
    ...
    filename1 = ['FTD_' stationN '_' dates2 '.mat']
    if exist(filename1,'file') % Check that the file exists
        ...
        error(['Cannot find a mat.file with an estimated greens function for stationpair ' stationN...
            '. Fileformat must be: FTD_' stationN '_' dates2 '.mat' ])
    end
